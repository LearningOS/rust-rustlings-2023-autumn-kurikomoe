...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        ...
        let r = slice[0].try_into().map_err(|_| IntoColorError::IntConversion)?;
        let g = slice[1].try_into().map_err(|_| IntoColorError::IntConversion)?;
        let b = slice[2].try_into().map_err(|_| IntoColorError::IntConversion)?;
        Ok(Self {
            red: r,
            green: g,
            blue: b,
